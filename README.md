...
| 확인 주기 | 상단 `cron` | 기본 `"*/5 * * * *"` 5분마다 (GitHub 최소 단위, 실제로는 몇 분 지연 가능) |
...
2. **폴링 사이의 급행 소환.** 누군가 조던을 한 번에 왕창 팔면 1→6이 몇십 분 만에 끝나기도 합니다. 기본 주기를 5분으로 잡았지만 GitHub 예약 실행은 몇 분씩 지연될 수 있고, 5분보다 짧게는 지원되지 않습니다. 이를 보완하는 것이 아래의 **빠른 알림 차선(Cloudflare Worker + 디스코드)**입니다. 진행도가 어느 방향으로든 변하면(상승뿐 아니라 **하락 = 그 사이 소환 추정**까지) 알림이 옵니다.
...
## 빠른 알림 차선 — Cloudflare Worker + 디스코드 (권장)

GitHub 차선(이메일 + 상태 페이지)과 별개로, **2분마다** 확인해서 변동 즉시 **디스코드 푸시 알림**을 보내는 고속 차선입니다. `fastlane/` 폴더의 `worker.js`와 `wrangler.toml`이 그것으로, Cloudflare Workers 무료 플랜으로 돌아갑니다 (하루 720회 확인 ≒ 무료 한도의 1% 미만). 참고로 diablo2.io 트래커의 진행도는 이제 d2emu의 자동 감지 데이터로 갱신되기 때문에, 빠르게 폴링하면 사실상 준실시간 감지가 됩니다.

설치는 동봉된 **cowork-prompt-v2.md** 를 Cowork에 붙여넣으면 자동으로 진행됩니다. 수동으로 하려면: Cloudflare 계정 → `npm i -g wrangler` → `wrangler login` → `wrangler kv namespace create STATE` 후 나온 id를 `wrangler.toml`에 기입 → `wrangler secret put DISCORD_WEBHOOK` (디스코드 채널 설정 → 연동 → 웹후크에서 URL 발급) → `wrangler deploy`. 디스코드 앱의 해당 채널 알림을 "모든 메시지"로 켜두면 휴대폰 푸시가 즉시 옵니다. 6/6 소환 시에는 @everyone 멘션으로 크게 울립니다.
